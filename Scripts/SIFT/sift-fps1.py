...
def main():
    ...
    print (f"{fileName.split('/')[-1]} fps: ", fps)
    ...
    # Just pick one keyframe
    eucDist = np.array(euclideanDistance)
    keyFramesIndices.append(np.argmax(eucDist))
    videoCap.set(cv2.CAP_PROP_POS_FRAMES,keyFramesIndices[0])
    ...
    print ('Time taken to run =', time.time() - t0, 'seconds' )
